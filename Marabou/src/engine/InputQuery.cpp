...
void InputQuery::incrementNumberOfVariables()
{
    _numberOfVariables++;
...
void InputQuery::dumpEquations() const
{
    printf( "Equations:\n" );
    for ( const auto &e : _equations )
    {
...
void InputQuery::dumpOutputVars() const
{
...
List<unsigned> InputQuery::outputVars() const
{
    List<unsigned> outputVarsList;
    for ( const auto &pair : _outputIndexToVariable )
        outputVarsList.append( pair.second );

    return outputVarsList;
}

void InputQuery::dumpInputVars() const
{
...
void InputQuery::dumpPLConstraints() const
{
    printf( "Constraints:\n" );
    String constraintString;
    for ( const auto &pl : _plConstraints )
    {
        pl->PiecewiseLinearConstraint::dump( constraintString );
...
void InputQuery::dump() const
{
    ...
    printf( "Variable bounds: (LB and UB of each variable)\n" );
    for ( unsigned i = 0; i < _numberOfVariables; ++i )
    {
        printf( "\t %u: [%s, %s]\n", i,
