...
bool AbsoluteValueConstraint::satisfied() const
{
    ...
    if ( FloatUtils::gt( bValue, 1 ) )
        return FloatUtils::gt( fValue, 1 );

    return FloatUtils::areEqual(fValue, bValue);
}
...
PiecewiseLinearCaseSplit AbsoluteValueConstraint::getNegativeSplit() const
{
    ...
    negativePhase.storeBoundTightening( Tightening( _b, 1.0, Tightening::UB ) );

    if ( _auxVarsInUse )
    {
        negativePhase.storeBoundTightening( Tightening( _negAux, 1.0, Tightening::UB ) );
    }
    else
    {
...
PiecewiseLinearCaseSplit AbsoluteValueConstraint::getPositiveSplit() const
{
    ...
    positivePhase.storeBoundTightening( Tightening( _b, 1.0, Tightening::LB ) );
...
void AbsoluteValueConstraint::updateVariableIndex( unsigned oldIndex, unsigned newIndex )
{
    ...
    ASSERT( oldIndex == _b || oldIndex == _f ||
            ( _auxVarsInUse && ( oldIndex == _posAux) ) );

    ASSERT( !existsLowerBound( newIndex ) &&
            !existsUpperBound( newIndex ) &&
            newIndex != _b && newIndex != _f && ( !_auxVarsInUse || ( newIndex != _posAux) ) );
...
void AbsoluteValueConstraint::getEntailedTightenings( List<Tightening> &tightenings ) const
{
    ...
    else if ( bLowerBound < 1 && bUpperBound >= 1 && FloatUtils::isZero( fLowerBound ) )
    {
    ...
    else if ( bLowerBound < 1 && bUpperBound >= 1 && fLowerBound > 1 )
    {
                                                                  ...
                                                                  &inputQuery )
{
    ...
    */
    ...
    //_negAux = _posAux + 1;
    inputQuery.setNumberOfVariables( _posAux + 1 );
...
String AbsoluteValueConstraint::serializeToString() const
{
    ...
    return _auxVarsInUse ?
        Stringf( "absoluteValue,%u,%u,%u,%u", _f, _b, _posAux ) :
...
void AbsoluteValueConstraint::fixPhaseIfNeeded()
{
    ...
    if ( existsLowerBound( _b ) && getLowerBound( _b ) >= 1 )
    {
    ...
    if ( existsUpperBound( _b ) && getUpperBound( _b ) < 1 )
    {
