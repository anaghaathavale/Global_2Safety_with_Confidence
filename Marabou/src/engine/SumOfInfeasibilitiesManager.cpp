...
void SumOfInfeasibilitiesManager::initializePhasePatternWithCurrentInputAssignment()
{
    ...
    */
    ...
    bool currentAssignmentObtained = false;
     for ( const auto &plConstraint : _plConstraints )
     {
              ...
              plConstraint->isActive() && !plConstraint->phaseFixed() )
         {
          ...
          if ( !_networkLevelReasoner->getConstraintsInTopologicalOrder().exists( plConstraint ) ) {
            if (!currentAssignmentObtained) {
              obtainCurrentAssignment();
              currentAssignmentObtained = true;
            }
