...
void PropertyParser::parse( const String &propertyFilePath, double &conf_from_user, double &epsilon_from_user )
{
...
//    double configValue;
    ...
    {  while(i<2)
       { 
            ...
            {	if (i==0){
                processSingleLine( line, conf_from_user);}
                if (i==1){
                processSingleLine( line, epsilon_from_user);}
		i++;
            }
        }
    }
...
void PropertyParser::processSingleLine(const String &line, double &configValue)
{
	// Convert the String to std::string
    std::string lineStr = line.ascii();

    // Use std::istringstream with the converted std::string
    ...
    String configName = tokens[0].c_str();
    configValue = extractScalar(tokens[1].c_str());
